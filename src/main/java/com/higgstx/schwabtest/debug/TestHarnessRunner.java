...
public class TestHarnessRunner implements CommandLineRunner
{
    ...
    private void handleMenuChoice(String choice, Scanner scanner) throws
            SchwabApiException
    {
        switch (choice)
        {
    ...
    private void handleApiException(SchwabApiException e)
    {
        ...
        {
            System.err.println(
                    "  Note: Authentication error - check credentials.");
        }

        if (e.isRateLimited())
        {
            System.err.println(
                    "  Note: Rate limited - please wait before retrying.");
        }
    ...
    private void testBulkHistoricalData(Scanner scanner) throws
            SchwabApiException
    {
        System.out.println("\n--- Testing Bulk Historical Data API ---");
        System.out.println(
                "This test uses the getBulkHistoricalData method to fetch 30 days of data for multiple symbols.");
        ...
        {
            System.out.println(
                    "Service not ready. Please ensure tokens are valid and try again.");
    ...
    private void displayBulkResults(String[] symbols,
            List<DailyPriceData> bulkData, long totalTime)
    {
        System.out.println("\n" + "=".repeat(70));
    ...
    private void displaySymbolResults(String symbol,
            List<DailyPriceData> symbolData)
    {
        ...
        long successCount = symbolData.stream().
                filter(DailyPriceData::isSuccess).count();
    ...
    private void displaySuccessfulDataSample(List<DailyPriceData> symbolData)
    {
        ...
        if (sample != null)
        {
            ...
            System.out.
                    println("    Volume: " + formatVolume(sample.getVolume()));
        }
    }
    ...
    private void displayPerformanceAnalysis(String[] symbols,
            List<DailyPriceData> bulkData, long totalTime)
    {
        ...
        System.out.println(
                "Data points per second: " + (bulkData.size() * 1000 / Math.max(
                totalTime, 1)));
    }
    ...
    private void automaticOAuth(Scanner scanner) throws SchwabApiException
    {
        ...
        System.out.println(
                "This will start a local HTTPS server to handle the OAuth callback automatically.");
        ...
        try
        {
            ...
            try (SchwabOAuthClient client = new SchwabOAuthClient(apiProperties))
            {
                ...
                performAutomaticOAuthFlow(scanner, authUrl, authCodeFuture,
                        client, redirectUri);
            }
    ...
    private SchwabApiProperties createApiProperties(String redirectUri) throws
            SchwabApiException
    {
    ...
    private void performAutomaticOAuthFlow(Scanner scanner, String authUrl,
            CompletableFuture<String> authCodeFuture,
            SchwabOAuthClient client, String redirectUri) throws
            SchwabApiException
    {
    ...
    private void saveTokensAndComplete(TokenResponse tokens) throws
            SchwabApiException
    {
        ...
        System.out.println(
                "You can now use the API endpoints (options 6, 7, or 8).");
    }
    ...
    private void manualOAuth(Scanner scanner) throws SchwabApiException
    {
        ...
        System.out.println(
                "This process requires manual URL copying and pasting.");
    ...
    private void performManualOAuthFlow(Scanner scanner,
            SchwabOAuthClient client) throws SchwabApiException
    {
        System.out.println(
                "============================================================");
        ...
        System.out.println(
                "4. Copy the COMPLETE redirect URL from your browser's address bar");
    ...
    private void testMarketData() throws SchwabApiException
    {
        ...
        {
            System.out.println(
                    "Service not ready. Please ensure tokens are valid.");
            System.out.println("Error: " + e.getMessage());
            return;
        }
    ...
    private void testMarketHours()
    {
        ...
        try
        {
            ...
            if (marketHours.getStatusCode() == 200)
            {
                ...
                System.out.println(
                        "Market hours data length: " + body.length() + " characters");
    ...
    private void testHistoricalData(Scanner scanner) throws SchwabApiException
    {
        ...
        try
        {
            var response = marketDataService.getPriceHistory(symbol, "month", 1,
                    "daily", 1);
    ...
    {
        System.out.println(
                "SUCCESS! Response Length: " + responseBody.length() + " characters");
        ...
        if (responseBody.contains("candles"))
        {
            ...
            {
                System.out.println("Sample Data: " + responseBody.substring(0,
                        500) + "...");
            }
            else
            {
    ...
    private void testAutomatedRefresh() throws SchwabApiException
    {
        ...
        {
            throw SchwabApiException.notFound(
                    "No tokens found. Please run OAuth authorization first (option 2).");
        }
        ...
        if (!currentTokens.isRefreshTokenValid())
        {
            throw SchwabApiException.tokenError(
                    "Refresh token is invalid - cannot force refresh");
        }
    ...
    {
        System.out.println("  Access token valid: " + tokens.
                isAccessTokenValid());
        System.out.println("  Refresh token valid: " + tokens.
                isRefreshTokenValid());
        System.out.println("  Access expires in: " + tokens.
    ...
    private void showConfigurationStatus()
    {
        ...
        System.out.println(
                "  App Key: " + (config.getAppKey() != null ? "LOADED" : "MISSING"));
        System.out.println(
                "  App Secret: " + (config.getAppSecret() != null ? "LOADED" : "MISSING"));
        ...
        System.out.println("  Market Data URL: " + config.getUrls().
                getMarketData());
        System.out.println("  Redirect URI: " + config.getDefaults().
                getRedirectUri());

        System.out.println("\nService Status:");
        System.out.println("  Token Status: " + marketDataService.
                getTokenStatus());
